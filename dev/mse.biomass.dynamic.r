...
#======================================================================
# MSE with small differences which are hardcoded ...
#======================================================================

mseBD2 <- function (OM, start, sr, srRsdl = FLQuant(1, dimnames = dimnames(window(rec(OM),
    start = start))), CV = 0.2, Ftar = 0.75, Btrig = 0.75, Fmin = Ftar * 0.1, 
    Blim = Btrig * 1e-04, bounds = NULL, aLag=1, maxHR=0.35, maxF=2, IEM="linear", 
    cthBias=1, srvBias=1, seed=1234, amod="bd", clag=5, slag=5){
	...
	bd <- as(OM, "FLBioDym")
    # abundance index bias NOTE: this should be before lognormal error
	index(bd) <- index(bd)*rnorm(length(index(bd)), srvBias, srvBias*0.1)
    # abundance index variability
    index(bd) <- index(bd)*rlnorm(prod(dim(index(bd))), 0, CV)
	# catch bias
	catch(bd) <- catch(bd)*runif(length(catch(bd)), cthBias*0.95, cthBias*1.05)
	...
	if(iYr %in% aYrs){
		...
		bd <- window(bd, end = an(dtaYr))
		# abundance index bias NOTE: this should be before lognormal error
		index(bd)[,dtaYrs] <- index(bd)[,dtaYrs]*rnorm(length(index(bd)[,dtaYrs]), srvBias, srvBias*0.1)
		# abundance index variability
		index(bd)[,dtaYrs] <- stock(OM)[,dtaYrs]*rlnorm(prod(dim(index(bd)[,dtaYrs])), 0, CV)
		# catch bias
		catch(bd)[,dtaYrs] <- computeCatch(OM)[, dtaYrs]
		...
		if(IEM=="linear" & cthBias!=1){
			#ie[ie<cmin & !is.na(ie)] <- cthBias
			ie[ie>cmax & !is.na(ie)] <- 1
			ie[ie<=cmax & !is.na(ie)] <- cthBias + (ie[ie<=cmax & !is.na(ie)] - cmin)*(1-cthBias)/(cmax-cmin)
		} else {
			ie[] <- cthBias			
		}
		ctrl@trgtArray[2*(1:(aLag+1)), "val", ] <- ctrl@trgtArray[2*(1:(aLag+1)), "val", ]/runif(length(ie), ie*0.95, ie*1.05)
...
mkScn <- function(Btrig=0.5, Ftar=0.75, maxHR=0.35, aLag=1, srvBias=1, cthBias=1, IEM="linear", slag=NA, clag=NA, am="bd", b0=0.5, runid=1, ref="missing", p=1){

	v <- data.frame(ref=ref, Btrig=Btrig, Ftar=Ftar, maxHR=maxHR, aLag=aLag, srvBias=srvBias, cthBias=cthBias, IEM=IEM, slag=slag, clag=clag, am=am, b0=b0, p=p, runid=runid, stringsAsFactors = FALSE)
	if(missing(ref)) v["ref"] <- paste(names(v[,-1]), v[,-1], collapse=",", sep="=")
	v
}


pvb <- function(stk, brp, perc, refy){
	mean(ssb(stk)[,refy]<c(ssb(brp)[,1])*perc)
}
